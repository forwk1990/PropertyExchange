...
#define kNetworkWillUseParameters NSStringFromSelector(@selector(networkManager:willUseParameters:))
...
- (void)buildInterceptorSteps{
    ...
    [self buildInterceptorStepWithSelector:@selector(networkManager:willUseParameters:)];
...
- (void)registerInterceptorTagert:(id)target{
    ...
    if([target respondsToSelector:@selector(networkManager:willUseParameters:)]){
        [self.interceptorSteps[kNetworkWillUseParameters] registerTarget:target];
    }
...
- (void)sendRequest{
    ...
    NSMutableDictionary *parameters = [NSMutableDictionary dictionaryWithDictionary:[self.delegate parameters]];
    [self.interceptorSteps[kNetworkWillUseParameters] invokeWithArguments:@[parameters]];
